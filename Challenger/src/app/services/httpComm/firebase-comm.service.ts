...
export class FirebaseCommService {
  ...
  url:string = 'https://challenger-dbd.firebaseio.com/';
  ...
  pushIndividual = (datatoEnter,table:string):Promise<Observable<challenge[]>>=>{
  ...
  getLastProperty = (property,condition,table:string)=>{
    ...
    return new Promise((resolve,reject)=>{
      ...
      this.comm.get(this.url+table+'.json').subscribe((data)=>{
        ...
        var previouselementCount = Object.keys(dataJSON).length-1;
        console.log(previouselementCount);
